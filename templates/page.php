...
/**
 * Function to render an array of Renderer items
 *
 * @param Renderer[] $items
 * @return string
 */
$render = function (array $items) {
    if (empty($items)) return '';
    return implode(PHP_EOL, array_map(function ($i) {
            return $i->render();
        }, $items)) . PHP_EOL;
};
 ...
 * @var Link[] $links_head
 */
$head_content .= $render($scripts_head ?? []);
$head_content .= $render($links_head ?? []);

$top_body .= $render($links_top_body ?? []);
$top_body .= $render($scripts_top_body ?? []);
$bottom_body .= $render($links_bottom_body ?? []);
$bottom_body .= $render($scripts_bottom_body ?? []);
